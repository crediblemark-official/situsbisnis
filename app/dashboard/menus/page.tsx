...
export default function MenusPage() {
    ...
    // Track previous slug to reset loading state when it changes
    const [prevMenuSlug, setPrevMenuSlug] = useState(menuSlug);
    ...
    if (menuSlug !== prevMenuSlug) {
        setPrevMenuSlug(menuSlug);
        setLoading(true);
    }
    ...
    useEffect(() => {
        ...
        async function fetchAll() {
            try {
                ...
                // Fetch menu items
                const menuRes = await fetch(`/api/menus?slug=${menuSlug}`);
                const menuData = await menuRes.json();
                
                if (!ignore) {
                    if (Array.isArray(pagesData)) {
                        setPublishedPages(pagesData.filter((p: any) => p.isPublished));
                    }
                    
                    const sorted = (menuData.items || []).sort((a: any, b: any) => a.order - b.order);
                    if (sorted.length === 0 && menuSlug === "main") {
                        setItems([
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const res = await fetch(`/api/menus/${menuSlug}`, {
                method: "PUT",
