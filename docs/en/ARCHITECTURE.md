...
Every module (except `shared`) follows a consistent layered structure:

```
modules/<domain>/
├── index.ts                # Facade (XxxClient) — public API for other modules
├── controllers/            # Orchestration layer — delegates to services
...
| **Dependency Cruiser** | 0 violations (490 modules, 1023 dependencies) |
