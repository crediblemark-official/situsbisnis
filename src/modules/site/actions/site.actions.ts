...
export async function updateSiteSettingsAction(body: any) {
    try {
        const { siteId, error } = await getApiContext(["admin", "owner"], { requireSite: true });
        ...
        const { customDomain: _, ...settingsData } = validation.data;
        ...
        console.error("[UPDATE_SITE_SETTINGS_ACTION] Error:", err);
        return { success: false, error: "Gagal memperbarui pengaturan situs" };
    }
}

export async function savePaymentSettingsAction(body: any) {
    try {
        ...
        await SiteClient.savePaymentSettings(siteId, validation.data);
        return { success: true };
    } catch (err: any) {
        console.error("[SAVE_PAYMENT_SETTINGS_ACTION] Error:", err);
